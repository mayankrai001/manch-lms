  ...
  <div class="py-16" :class="isAdmin ? 'bg-amber-50/30' : ''">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-primary-dark mb-2">Our Expert Faculty</h1>
        <p class="text-gray-500 text-sm">Meet the educators behind Manch Science Classes</p>
        <!-- Admin mode badge -->
      ...
      <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <div v-for="i in 4" :key="i" class="bg-white rounded-2xl shadow-sm overflow-hidden animate-pulse">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <div
          ...
          :class="[
            ...
            isAdmin ? 'ring-2 ring-amber-200 hover:ring-amber-400 hover:shadow-lg' : 'hover:shadow-md'
          ]"
        >
          <!-- Image Area -->
          <div class="relative h-52 bg-gray-100 group">
            <img
            ...
            <div v-else class="h-full w-full flex flex-col items-center justify-center text-gray-300">
            ...
            >
              ...
              <button
                ...
                title="Change photo"
              >
                <svg v-if="!member._uploading" class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <svg v-else class="h-5 w-5 text-white animate-spin" fill="none" viewBox="0 0 24 24">
            ...
            <!-- Upload progress bar -->
            <div v-if="member._uploadProgress > 0 && member._uploadProgress < 100" class="absolute bottom-0 left-0 right-0 h-1 bg-gray-200">
              <div class="h-full bg-blue-500 transition-all" :style="{ width: member._uploadProgress + '%' }"></div>
            </div>
          </div>
          ...
          <div class="p-5 text-center space-y-1.5">
            ...
            <div>
              <input
                ...
                @blur="saveField(member, 'name', $event.target.value)"
                placeholder="Faculty Name"
                class="w-full text-center text-lg font-bold text-gray-800 bg-transparent border-b-2 border-transparent hover:border-amber-300 focus:border-amber-400 focus:outline-none transition-colors rounded px-1 py-0.5"
              />
              <h3 v-else class="text-lg font-bold text-gray-800">{{ member.name || 'Faculty Name' }}</h3>
            </div>
            ...
            <div>
              <input
                ...
                @blur="saveField(member, 'subject', $event.target.value)"
                placeholder="Subject / Specialization"
                class="w-full text-center text-sm font-semibold text-secondary bg-transparent border-b-2 border-transparent hover:border-amber-300 focus:border-amber-400 focus:outline-none transition-colors px-1 py-0.5"
              />
              <p v-else class="text-sm font-semibold text-secondary">{{ member.subject || 'Subject' }}</p>
            </div>
            ...
            <div>
              <input
                ...
                @change="saveField(member, 'qualification', $event.target.value)"
                @blur="saveField(member, 'qualification', $event.target.value)"
                placeholder="Qualification"
                class="w-full text-center text-sm text-gray-600 bg-transparent border-b-2 border-transparent hover:border-amber-300 focus:border-amber-400 focus:outline-none transition-colors px-1 py-0.5"
              />
              <p v-else class="text-sm text-gray-600">{{ member.qualification || 'Qualification' }}</p>
            </div>
            ...
            <div>
              <input
                ...
                @change="saveField(member, 'experience', $event.target.value)"
                @blur="saveField(member, 'experience', $event.target.value)"
                placeholder="e.g., 10+ Years Experience"
                class="w-full text-center text-sm text-gray-500 bg-transparent border-b-2 border-transparent hover:border-amber-300 focus:border-amber-400 focus:outline-none transition-colors px-1 py-0.5"
              />
              <p v-else class="text-sm text-gray-500">{{ member.experience || 'Experience' }}</p>
            </div>
        ...
        <div
          v-if="isAdmin"
          @click="addNewMember"
          :class="['border-2 border-dashed rounded-2xl flex flex-col items-center justify-center h-72 cursor-pointer transition-all', adding ? 'border-amber-400 bg-amber-50' : 'border-gray-300 hover:border-amber-400 hover:bg-amber-50/50']"
        >
          <svg v-if="!adding" class="h-10 w-10 text-gray-400 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4" />
          </svg>
          <svg v-else class="h-8 w-8 text-amber-500 animate-spin mb-2" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
          </svg>
          <span class="text-sm text-gray-500 font-medium">{{ adding ? 'Adding...' : 'Add Faculty' }}</span>
        </div>
      </div>
    ...
    <div v-if="deleteTarget" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <p class="text-sm text-gray-500 mb-5">
          This will permanently remove <strong>{{ deleteTarget.name || 'this member' }}</strong> from the faculty list.
        </p>
        <div class="flex gap-3">
          ...
          <button @click="handleDeleteMember" :disabled="deleting" class="flex-1 py-2 bg-red-500 hover:bg-red-600 disabled:opacity-60 text-white rounded-xl text-sm font-medium transition flex items-center justify-center gap-2">
  ...
  methods: {
    ...
    async handleDeleteMember() {
      ...
      try {
        ...
        this.faculty = this.faculty.filter(m => m.id !== this.deleteTarget.id)
